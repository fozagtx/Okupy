...
3. The Eve agent turns the profile into an Exa search for current, local opportunities.
...
Create a Render Blueprint from `render.yaml`, then set the Exa, Eve, Composio, and Photon secrets. The API uses a persistent disk for memory; the Photon private service keeps the iMessage stream alive.
